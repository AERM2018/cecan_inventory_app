DROP PROCEDURE public.reserve_medicines_to_prescription(IN prescrip_id uuid, IN reason character varying);
CREATE OR REPLACE PROCEDURE public.reserve_medicines_to_prescription(IN prescrip_id uuid, IN reason character varying)
 LANGUAGE plpgsql
AS $procedure$
				...
				loop
					...
					if stock.pieces_left >= pieces_needed then
						update pharmacy_stocks 
							...
							pieces_left = stock.pieces - (stock.pieces_used + pieces_needed)
							where id = stock.id; 
						pieces_needed := 0;
					else
						update pharmacy_stocks 
