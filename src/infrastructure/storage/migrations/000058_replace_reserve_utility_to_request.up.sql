DROP PROCEDURE public.reserve_utility_to_request(IN req_id uuid);
				...
				loop
					...
					if stock.quantity_parsed_left >= pieces_needed then
						update storehouse_stocks 
							set quantity_presentation_used = (stock.quantity_parsed_used + pieces_needed) / storehouse_utility_storehouse_request.quantity_per_unit,
                                quantity_presentation_left = quantity_presentation - ((stock.quantity_parsed_used + + pieces_needed) / storehouse_utility_storehouse_request.quantity_per_unit),
								quantity_parsed_used = stock.quantity_parsed_used + pieces_needed,
								quantity_parsed_left = quantity_parsed - (stock.quantity_parsed_used + pieces_needed)
							where id = stock.id; 
						pieces_needed := 0;
					else
						update storehouse_stocks 
